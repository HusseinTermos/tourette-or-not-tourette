...
class AudioEngine(QtCore.QObject):
    # meters + status signals
    meters = QtCore.Signal(float, float)   # in_rms, out_rms
    xruns  = QtCore.Signal(int)            # total xruns (dropouts/overruns)
    error  = QtCore.Signal(str)            # fatal errors
    ...
    @staticmethod
    def choose_channels(in_dev_index, out_dev_index):
        # Try mono first; if either device can't do mono, try stereo.
        def caps(idx, want_input):
        ...
        in_ch_max = caps(in_dev_index, True)
        out_ch_max = caps(out_dev_index, False)
        if in_ch_max >= 1 and out_ch_max >= 1:
            return 1
        if in_ch_max >= 2 and out_ch_max >= 2:
            return 2
        raise RuntimeError("Selected devices have no compatible channel count (need ≥1 or both ≥2).")

    def start(self):
        if self._stream is not None:
            return
        channels = self.choose_channels(self.in_dev_index, self.out_dev_index)
        dtype = "float32"
        ...
        def audio_cb(indata, outdata, frames, time_info, status):
            if status:
                # Count any non-OK status as xrun (safe approximation)
                self._xruns += 1
                self.xruns.emit(self._xruns)

            # Queue copy for scoring worker
            try:
                self._in_q.put_nowait(indata.copy())
            except queue.Full:
                pass  # drop to stay realtime

            # Pull processed chunk; if none, output silence
            try:
    ...
    def _worker_loop(self):
        ...
        while not self._stop_flag.is_set():
            try:
                chunk = self._in_q.get(timeout=0.1)  # ndarray float32, shape (frames, channels)
            except queue.Empty:
                continue
            if chunk is None:
                break

            # Prepare audio as base64: use the raw float32 PCM bytes of this block.
            # (If your API expects int16 @ 16kHz, tell me and I’ll add resampling/formatting.)
...
class MainWindow(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Virtual Mic Filter (HTTP Score → Mute < threshold)")
        ...
        self.url_edit = QtWidgets.QLineEdit("http://127.0.0.1:8000/score")
        ...
        self.sr_spin.setValue(48000)   # many devices prefer 48k

        self.block_spin = QtWidgets.QSpinBox()
        self.block_spin.setRange(128, 8192)
        self.block_spin.setSingleStep(128)
        ...
        for m in (self.in_meter, self.out_meter):
            m.setRange(0, 1000)  # scale RMS → 0..1000

        self.xruns_label = QtWidgets.QLabel("XRUNs: 0")
        ...
        self._meter_timer.timeout.connect(lambda: None)  # keep event loop alive when idle
    ...
    def populate_devices(self):
        ...
        for _, label in self._in_map:
            self.in_combo.addItem(label)
        for _, label in self._out_map:
            self.out_combo.addItem(label)

        # Preselect VirtualMic if present
    ...
    def _current_in_index(self):
        if self.in_combo.currentIndex() < 0: return None
        return self._in_map[self.in_combo.currentIndex()][0]

    def _current_out_index(self):
        if self.out_combo.currentIndex() < 0: return None
        return self._out_map[self.out_combo.currentIndex()][0]
    ...
    def start_engine(self):
        ...
        in_idx = self._current_in_index()
        out_idx = self._current_out_index()

        self.engine = AudioEngine(url, thr, sr, block, in_idx, out_idx)
        self.engine.meters.connect(self.on_meters)
        self.engine.xruns.connect(self.on_xruns)
        ...
        for w in [self.in_combo, self.out_combo, self.refresh_btn,
                  self.url_edit, self.thr_spin, self.sr_spin, self.block_spin]:
            w.setEnabled(False)
    ...
    def stop_engine(self):
        ...
        self.xruns_label.setText("XRUNs: 0")
        # Unlock config
        for w in [self.in_combo, self.out_combo, self.refresh_btn,
                  self.url_edit, self.thr_spin, self.sr_spin, self.block_spin]:
            w.setEnabled(True)
    ...
    def on_meters(self, in_rms, out_rms):
        # simple log-ish scale: clamp & sqrt-ish mapping
        def scale(x):
            x = max(1e-6, min(1.0, x * 10.0))  # boost quiet speech
            return int(1000 * np.sqrt(x))
        self.in_meter.setValue(scale(in_rms))
        self.out_meter.setValue(scale(out_rms))

    @QtCore.Slot(int)
    def on_xruns(self, n):
        self.xruns_label.setText(f"XRUNs: {n}")
